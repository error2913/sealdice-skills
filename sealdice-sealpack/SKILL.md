...
[package]
id = "your-name/your-plugin"   # 必填，author/package，两段各只能含字母数字_-，≤64 字符
name = "你的插件名"             # 必填
...
# 也可用 --contents scripts,decks,reply 指定类别，--category 指定商店分类
```
...
sealpack version set 1.1.0        # 或 version patch/minor/major
...
`npm run pack:sealpack`（打 dist 包）、`npm run pack:release`（本体 JS + 带版本豹包）。
