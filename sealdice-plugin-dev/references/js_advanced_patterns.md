...
3. **状态全部放 ext.storage**（只收字符串，复杂结构 JSON 序列化），常驻循环每次从
   存储读取：插件重载后循环可继续运行，任务状态不丢失；`onLoad` 打印恢复日志便于确认。
   ```js
...
- 使用方：**必须在元数据头声明依赖**，保证提供方先注册、缺失/版本不符时拒载：
  ...
  ```
  注意作者名可含 `&` 等字符，作者、插件名、版本之间始终用英文冒号分隔，
  例如 `作者&合作者:插件名:>=1.0.0`。
- 调用时仍要判空防御（依赖声明缺失、加载顺序异常、提供方被卸载时直接提示）：
  ...
  ```
- 命名空间要唯一，避免与其他插件命名冲突；提供方热重载后 `globalThis` 引用会更新，
...
1. **空数组随机索引**：`arr[Math.floor(Math.random() * arr.length)]` 在 `arr.length === 0`
   时得到 `undefined` 并导致后续逻辑异常。随机取值前先判空：
   ```js
   if (!arr || arr.length === 0) return fallbackCode;  // 题库为空时回退自动生成
   ```
2. **失败/成功返回类型不一致**：`getQQLevel()` 失败 `return 0`、成功
   `return {qqLevel, nickname}`，调用方解构时报错。失败分支要返回同构对象：
   ```js
   return { qqLevel: 0, nickname: '未知' };
   ```
3. **日志对象不一定存在**：某些环境 `logger` 未定义，`logger.error(...)` 直接抛错；
   兜底用 `console.error`。
4. **移除调试输出**：发布前清理 `console.log(JSON.stringify(...))` 等调试输出。
5. **截图失败要显示 HTTP 状态码与响应内容预览**：只提示「截图失败」无法排查后端
   问题；返回非 200 时把状态码 + 前 120 字符响应体带出来。
6. **不要校验图片拍摄时间**：QQ 会清空图片 EXIF 时间，基于 EXIF 的校验上线后被
   回滚；需要时间信息时应以消息时间或服务端记录为准。
7. **长耗时请求要提示用户**：AI 识别类接口响应可能长达数分钟，超时时间需设置得足够大并回复
   「模型响应慢，请等待」，避免用户误以为机器人无响应。
