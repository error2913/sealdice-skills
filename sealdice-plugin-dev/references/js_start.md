...
例如，当插件使用了新版本 <Badge type="tip" text="LatestVersion"/> 新增的 API，应将目标海豹版本填写为该 API 实际引入的核心版本。不满足兼容条件时，核心会向骰主提示版本不兼容并拒绝加载。

文档或示例尚未填入正式版本时会使用 `vA.B.C` 占位；写入插件元数据时应去掉 `v` 并替换为实际版本：

```javascript
// A.B.C 应替换为所用 API 实际引入的核心版本。
// @sealVersion  A.B.C
```

::: info 兼容的海豹版本

`@sealVersion` 是 JS API 的核心版本兼容声明，当前没有另一个独立的 `@apiVersion` 字段。使用哪个版本新增的 API，就至少声明哪个核心版本；<Badge type="tip" text="LatestVersion"/> 新增接口应声明对应接口引入时的实际核心版本。

海豹会尝试在兼容的情况下尽可能地加载插件，这是由海豹核心自动处理的。

该字段支持 SemVer 约束。例如 `@sealVersion >=1.4.6` 会严格检查当前核心版本；不带比较符号的 `@sealVersion 1.4.6` 会使用核心维护的 JS API 兼容版本集合判断。除非插件确实要求所有后续核心版本，否则推荐填写引入所用 API 的最低目标版本。
